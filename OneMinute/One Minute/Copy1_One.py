...
root = Tk()
root.iconbitmap("C:/Users/User/Desktop/Code/OneMinute/one minute.ico")
...
def show_file():  # Function for show events button
    filedialog.askopenfilename(initialdir='C:/Users/User/Documents', title=('All files', '*.*'))
...
def member_details_button():  # Function for show member details button
    ...
    def window_edit():
        ...
        editing_root.iconbitmap('C:/Users/User/Desktop/Code/OneMinute/one minute.ico')
        ...
        if len(editing_details) == 0 or id_entry_chamber.get() == str():
            ...
            root_info.iconbitmap('C:/Users/User/Desktop/Code/OneMinute/one minute.ico')
...
def open_writing_window():  # Function for Start writing button
    ...
    side_frame = LabelFrame(root_writing, text="Put yoh shot here", font=("Helvetica", 10))
    side_frame.grid(row=0,column=1)
    saveButton = Button(root_writing, text="Save", font=("Helvetica", 10))
    saveButton.grid(row=1, column=0, columnspan=5)

    #Packing in the frames
    main_text_box = Text(side_frame_right, height=20, width=20)
    main_text_box.grid(row=0, column=0)
    majorTextBox = Text(side_frame, height=35, width=60)
